...
composants_actifs = [True, False, False, False, False] # [dessin levé, gomme, ligne droite, rectangle, remplissage rectangulaire]
tracé = []
liste_des_tracés = []
...
try:
    ...
    taille_du_multiplicateur_choix = 1
...
type_de_performance = simpledialog.askstring("Type du fichier","Tapez 1 pour graphique précis, 2 pour graphique fast")
...
def premier_point(event):
    global clic_x, clic_y, point_de_départ_x, point_de_départ_y, color, composants_actifs, tracé, liste_des_tracés, activité
    activité = 1
...
def en_train_de_dessiner(event):
    ...
    # Ligne droite
        ...
        identifiant_ligne_droite_temporaire = zone_de_dessin.create_line(point_de_départ_x, point_de_départ_y, delta_x, delta_y, fill = color, width = 1, dash = (1,1))
    ...
    for x in range(img.size[0]):
        for y in range(img.size[1]):
            ...
            try:
                ...
                if pixel == pixel_suivant and pixel != (255, 255, 255):
                    identifiant_de_la_ligne = zone_de_dessin.create_line(x, y, x + 1, y, fill = '#%02x%02x%02x' % pixel, width = 1)
                    tracé.append((identifiant_de_la_ligne, [(x, y), (x + 1, y)], '#%02x%02x%02x' % pixel))
                elif pixel != (255, 255, 255):
                    if tracé != []:
                        liste_des_tracés.append(tracé)
                    tracé = []
                    identifiant_de_la_ligne = zone_de_dessin.create_line(x, y, x + 0.9, y, fill = '#%02x%02x%02x' % pixel, width = 1)
                    tracé.append((identifiant_de_la_ligne, [(x, y), (x + 0.9, y)], '#%02x%02x%02x' % pixel))
            except Exception:
                if tracé != []:
                    liste_des_tracés.append(tracé)
                tracé = []
...
def image_en_contour():
    ...
    img.thumbnail((230, 450))
    img = img.quantize(colors = 12, method = 2).convert("RGB")
    img = img.convert("L")
...
def générer_le_fichier_Gcode():
    global type_du_fichier_d_exportation, nom_du_fichier_d_exportation, nombre_de_fichiers_générés
    ...
    if len(nom_du_fichier_d_exportation) != 0:
        ...
        nom_base = "export"
    if type_du_fichier_d_exportation == str(2):
        extension = ".gcode"
    else:
        extension = ".txt"
    for couleur in liste_des_couleurs:
        ...
        try:
            with open(nom_du_fichier, "w") as f:
                ...
                f.write("G90\n") 
                for tracé in liste_des_tracés:            
                    if tracé[0][2] == couleur:
                        f.write(f"G0 X{round(tracé[0][1][0][0] / taille_du_multiplicateur_choix, 2)} Y{round(tracé[0][1][0][1] / taille_du_multiplicateur_choix, 2)}\n")
                        f.write("G0 Z0\n")
                        for élément in tracé:
                            f.write(f"G0 X{round(élément[1][1][0] / taille_du_multiplicateur_choix, 2)} Y{round(élément[1][1][1] / taille_du_multiplicateur_choix, 2)}\n")
                        f.write("G0 Z5\n")
                f.write("G0 X0 Y0 Z5\n")
        except IOError as e:
...
# Racourcis claviers
fenêtre.bind("<q>", lambda event : noir())
fenêtre.bind("<Q>", lambda event : noir())
fenêtre.bind("<s>", lambda event : bleu())
fenêtre.bind("<S>", lambda event : bleu())
fenêtre.bind("<g>", lambda event : vert())
fenêtre.bind("<G>", lambda event : vert())
fenêtre.bind("<h>", lambda event : rouge())
fenêtre.bind("<H>", lambda event : rouge())
fenêtre.bind("<z>", lambda event : cyan())
fenêtre.bind("<Z>", lambda event : cyan())
fenêtre.bind("<r>", lambda event : jaune())
fenêtre.bind("<R>", lambda event : jaune())
fenêtre.bind("<t>", lambda event : magenta())
fenêtre.bind("<T>", lambda event : magenta())
fenêtre.bind("<j>", lambda event : autre_couleur_choix())
fenêtre.bind("<J>", lambda event : autre_couleur_choix())
fenêtre.bind("<k>", lambda event : gomme())
fenêtre.bind("<K>", lambda event : gomme())
fenêtre.bind("<l>", lambda event : ligne())
fenêtre.bind("<L>", lambda event : ligne())
fenêtre.bind("<m>", lambda event : rectangle())
fenêtre.bind("<M>", lambda event : rectangle())
fenêtre.bind("<w>", lambda event : remplissage_rectangulaire())
fenêtre.bind("<W>", lambda event : remplissage_rectangulaire())
fenêtre.bind("<x>", lambda event : générer_le_fichier_Gcode())
fenêtre.bind("<X>", lambda event : générer_le_fichier_Gcode())
fenêtre.bind("<v>", lambda event : retour())
fenêtre.bind("<V>", lambda event : retour())
fenêtre.bind("<n>", lambda event : changement_de_type_de_performance())
fenêtre.bind("<N>", lambda event : changement_de_type_de_performance())
fenêtre.bind("<y>", lambda event : image_en_couleur())
fenêtre.bind("<Y>", lambda event : image_en_couleur())
fenêtre.bind("<u>", lambda event : image_en_contour())
fenêtre.bind("<U>", lambda event : image_en_contour())
fenêtre.bind("<i>", lambda event : maj_palette_de_couleur())
fenêtre.bind("<I>", lambda event : maj_palette_de_couleur())
fenêtre.bind("<o>", lambda event : augmenter_taille())
fenêtre.bind("<O>", lambda event : augmenter_taille())
fenêtre.bind("<p>", lambda event : diminuer_taille())
fenêtre.bind("<P>", lambda event : diminuer_taille())
