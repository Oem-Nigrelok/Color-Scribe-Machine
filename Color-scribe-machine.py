...
#-------------------------------ATTENTION-------------------------------#
#-------------------------------COLOR SCRIBE MACHINE GROUP-------------------------------#
#-------------------------------VOUS RECOMMANDE VIVEMENT-------------------------------#
#-------------------------------DE PLACER ICI VOTRE PALETTE DE COULEUR QUOTIDIENNE-------------------------------#
#-------------------------------POUR NE PAS AVOIR A LA REECRIRE-------------------------------#
#-------------------------------ELLE RESTERA DANS CE FICHIER TANT QUE CE FICHIER EXISTE-------------------------------#
liste_de_couleurs_utilisables_de_base = [(0, 255, 255), (255, 255, 0), (255, 0, 255)] # A modifier (sous format : (r, g, b), (r, g, b), ...)
print("Pour votre confort d'artiste, suivez les conseils de la ligne 7")
...
# Création de la fenêtre de couleur, fenêtre de palette et fenêtre de couleur globale
fenêtre_de_couleur = tk.Canvas(fenêtre, bg = color, width = 249, height = 26)
fenêtre_de_couleur.place(x = taille_de_la_feuille_x + 85, y = 26)
fenêtre_de_palette_1 = tk.Canvas(fenêtre, bg = color, width = 26, height = 26)
fenêtre_de_palette_1.place(x = taille_de_la_feuille_x + 2, y = 366)
fenêtre_de_palette_2 = tk.Canvas(fenêtre, bg = color, width = 26, height = 26)
fenêtre_de_palette_2.place(x = taille_de_la_feuille_x + 30, y = 366)
fenêtre_de_palette_3 = tk.Canvas(fenêtre, bg = color, width = 26, height = 26)
fenêtre_de_palette_3.place(x = taille_de_la_feuille_x + 58, y = 366)
fenêtre_de_palette_4 = tk.Canvas(fenêtre, bg = color, width = 26, height = 26)
fenêtre_de_palette_4.place(x = taille_de_la_feuille_x + 86, y = 366)
fenêtre_de_palette_5 = tk.Canvas(fenêtre, bg = color, width = 26, height = 26)
fenêtre_de_palette_5.place(x = taille_de_la_feuille_x + 114, y = 366)
fenêtre_de_palette_6 = tk.Canvas(fenêtre, bg = color, width = 26, height = 26)
fenêtre_de_palette_6.place(x = taille_de_la_feuille_x + 142, y = 366)
fenêtre_de_palette_7 = tk.Canvas(fenêtre, bg = color, width = 26, height = 26)
fenêtre_de_palette_7.place(x = taille_de_la_feuille_x + 170, y = 366)
fenêtre_de_couleur_globale = tk.Canvas(fenêtre, bg = "white", width = taille_de_la_feuille_x, height = 6)
fenêtre_de_couleur_globale.place(x = 1, y = taille_de_la_feuille_y + 10)
...
# Couleurs
def noir():
    global color, fenêtre_de_couleur, hex_couleur
    if activité == 1:
        return
    color = "#000000"
    fenêtre_de_couleur["bg"] = color
    hex_couleur["text"] = f"{color}"
...
def fermer_le_choix_autre_couleur():
    global color, entrée_texte_couleur, bouton_valider_le_choix_autre_couleur, fenêtre_de_couleur, hex_couleur
    if activité == 1:
        return
    try:
...
# Utiliser la pallette de couleur
def utiliser_couleur_palette_suivante():
    global liste_de_couleurs_utilisables, hex_couleur, color, fenêtre_de_couleur, fenêtre_de_palette_1, fenêtre_de_palette_2, fenêtre_de_palette_3, fenêtre_de_palette_4, fenêtre_de_palette_5, fenêtre_de_palette_6, fenêtre_de_palette_7
    if activité != 1:
        if not liste_de_couleurs_utilisables:
            maj_palette_de_couleur()
        palette = [c for c in liste_de_couleurs_utilisables if c != (255, 255, 255)]
        if not palette:
            return
        couleur_actuelle = tuple(int(color.lstrip('#')[i : i + 2], 16) for i in (0, 2, 4))
        if couleur_actuelle in palette:
            color = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 1) % len(palette)]
            fenêtre_de_palette_1["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 2) % len(palette)]
            fenêtre_de_palette_2["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 1) % len(palette)]
            fenêtre_de_palette_3["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 0) % len(palette)]
            fenêtre_de_palette_4["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 1) % len(palette)]
            fenêtre_de_palette_5["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 2) % len(palette)]
            fenêtre_de_palette_6["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 3) % len(palette)]
            fenêtre_de_palette_7["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 4) % len(palette)]
        else:
            color = '#%02x%02x%02x' % palette[0]
            couleur_actuelle = tuple(int(color.lstrip('#')[i : i + 2], 16) for i in (0, 2, 4))  # ← recalcul
            fenêtre_de_palette_1["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 3) % len(palette)]
            fenêtre_de_palette_2["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 2) % len(palette)]
            fenêtre_de_palette_3["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 1) % len(palette)]
            fenêtre_de_palette_4["bg"] = '#%02x%02x%02x' % palette[palette.index(couleur_actuelle)]
            fenêtre_de_palette_5["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 1) % len(palette)]
            fenêtre_de_palette_6["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 2) % len(palette)]
            fenêtre_de_palette_7["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 3) % len(palette)]
        fenêtre_de_couleur["bg"] = color
        hex_couleur["text"] = f"{color}"

def utiliser_couleur_palette_précédente():
    global liste_de_couleurs_utilisables, hex_couleur, color, fenêtre_de_couleur, fenêtre_de_palette_1, fenêtre_de_palette_2, fenêtre_de_palette_3, fenêtre_de_palette_4, fenêtre_de_palette_5, fenêtre_de_palette_6, fenêtre_de_palette_7
    if activité != 1:
        if not liste_de_couleurs_utilisables:
            maj_palette_de_couleur()
        ...
        if couleur_actuelle in palette:
            color = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 1) % len(palette)]
            fenêtre_de_palette_1["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 4) % len(palette)]
            fenêtre_de_palette_2["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 3) % len(palette)]
            fenêtre_de_palette_3["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 2) % len(palette)]
            fenêtre_de_palette_4["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 1) % len(palette)]
            fenêtre_de_palette_5["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle)) % len(palette)]
            fenêtre_de_palette_6["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 1) % len(palette)]
            fenêtre_de_palette_7["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 2) % len(palette)]
        else:
            color = '#%02x%02x%02x' % palette[-1]
            couleur_actuelle = tuple(int(color.lstrip('#')[i : i + 2], 16) for i in (0, 2, 4))  # ← recalcul
            fenêtre_de_palette_1["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 3) % len(palette)]
            fenêtre_de_palette_2["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 2) % len(palette)]
            fenêtre_de_palette_3["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 1) % len(palette)]
...
def ajouter_couleur_à_la_palette():
    global liste_de_couleurs_utilisables, color, fenêtre_de_palette_1, fenêtre_de_palette_2, fenêtre_de_palette_3, fenêtre_de_palette_4, fenêtre_de_palette_5, fenêtre_de_palette_6, fenêtre_de_palette_7
    if tuple(int(color.lstrip('#')[i : i + 2], 16) for i in (0, 2, 4)) not in liste_de_couleurs_utilisables:
        couleur_actuelle = tuple(int(color.lstrip('#')[i : i + 2], 16) for i in (0, 2, 4))
        liste_de_couleurs_utilisables.append(couleur_actuelle)
        palette = [c for c in liste_de_couleurs_utilisables if c != (255, 255, 255)]
        fenêtre_de_palette_1["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 3) % len(palette)]
        fenêtre_de_palette_2["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 2) % len(palette)]
        fenêtre_de_palette_3["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 1) % len(palette)]
        fenêtre_de_palette_4["bg"] = color
        fenêtre_de_palette_5["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 1) % len(palette)]
        fenêtre_de_palette_6["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 2) % len(palette)]
        fenêtre_de_palette_7["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 3) % len(palette)]
        
# Retirer la couleur actuelle à la palette
def retirer_couleur_à_la_palette():
    global liste_de_couleurs_utilisables, color, fenêtre_de_palette_1, fenêtre_de_palette_2, fenêtre_de_palette_3, fenêtre_de_palette_4, fenêtre_de_palette_5, fenêtre_de_palette_6, fenêtre_de_palette_7
    if tuple(int(color.lstrip('#')[i : i + 2], 16) for i in (0, 2, 4)) in liste_de_couleurs_utilisables and len(liste_de_couleurs_utilisables) != 1:
        palette = [c for c in liste_de_couleurs_utilisables if c != (255, 255, 255)]
        couleur_ancienne = tuple(int(color.lstrip('#')[i : i + 2], 16) for i in (0, 2, 4))
        index_suivant = (palette.index(couleur_ancienne) + 1) % len(palette)
        couleur_actuelle = palette[index_suivant]
        liste_de_couleurs_utilisables.remove(couleur_ancienne)
        palette = [c for c in liste_de_couleurs_utilisables if c != (255, 255, 255)]
        if not palette:
            return
        fenêtre_de_palette_1["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 3) % len(palette)]
        fenêtre_de_palette_2["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 2) % len(palette)]
        fenêtre_de_palette_3["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 1) % len(palette)]
        fenêtre_de_palette_4["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle)) % len(palette)]
        fenêtre_de_palette_5["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 1) % len(palette)]
        fenêtre_de_palette_6["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 2) % len(palette)]
        fenêtre_de_palette_7["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 3) % len(palette)]
...
def maj_palette_de_couleur():
    global liste_de_couleurs_utilisables_de_base, list_color_base_affichage, list_color_affichage, list_color_mel_affichage, liste_de_couleurs_utilisables_ajoutées, liste_de_couleurs_utilisables, fenêtre_de_palette_1, fenêtre_de_palette_2, fenêtre_de_palette_3, fenêtre_de_palette_4, fenêtre_de_palette_5, fenêtre_de_palette_6, fenêtre_de_palette_7
    if activité != 1:
        try:
            test = list(liste_de_couleurs_utilisables_de_base)
            test = (round((1 - ((1 - liste_de_couleurs_utilisables_de_base[0][0] / 255) + (1 - liste_de_couleurs_utilisables_de_base[0][0] / 255)) / 2) * 255), round((1 - ((1 - liste_de_couleurs_utilisables_de_base[0][1] / 255) + (1 - liste_de_couleurs_utilisables_de_base[0][1] / 255)) / 2) * 255), round((1 - ((1 - liste_de_couleurs_utilisables_de_base[0][2] / 255) + (1 - liste_de_couleurs_utilisables_de_base[0][2] / 255)) / 2) * 255))
        except Exception:
            liste_de_couleurs_utilisables_de_base = [(0, 255, 255), (255, 255, 0), (255, 0, 255)]
        if liste_de_couleurs_utilisables_de_base and not liste_de_couleurs_utilisables_ajoutées and not liste_de_couleurs_utilisables:
            if len(liste_de_couleurs_utilisables_de_base) == 1 and set(liste_de_couleurs_utilisables_de_base) != {(255, 255, 255)}:
                liste_de_couleurs_utilisables = []
                liste_de_couleurs_utilisables_ajoutées = []
                liste_de_couleurs_utilisables.extend(liste_de_couleurs_utilisables_de_base)
                list_color_base_affichage["text"] = f"Palette rentrée : {liste_de_couleurs_utilisables_de_base}"
                list_color_affichage["text"] = f"Palette de dessin : {liste_de_couleurs_utilisables}"
                list_color_mel_affichage["text"] = f"Mélanges calculés : {liste_de_couleurs_utilisables_ajoutées}"
                couleur_actuelle = tuple(int(color.lstrip('#')[i : i + 2], 16) for i in (0, 2, 4))
                palette = [c for c in liste_de_couleurs_utilisables if c != (255, 255, 255)]
                fenêtre_de_palette_1["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 3) % len(palette)]
                fenêtre_de_palette_2["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 2) % len(palette)]
                fenêtre_de_palette_3["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) - 1) % len(palette)]
                fenêtre_de_palette_4["bg"] = '#%02x%02x%02x' % couleur_actuelle
                fenêtre_de_palette_5["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 1) % len(palette)]
                fenêtre_de_palette_6["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 2) % len(palette)]
                fenêtre_de_palette_7["bg"] = '#%02x%02x%02x' % palette[(palette.index(couleur_actuelle) + 3) % len(palette)] 
                ...
                return
            elif set(liste_de_couleurs_utilisables_de_base) == {(0, 0, 0), (255, 255, 255)}:
                liste_de_couleurs_utilisables = []
                liste_de_couleurs_utilisables_ajoutées = []
                liste_de_couleurs_utilisables.extend(liste_de_couleurs_utilisables_de_base)
                list_color_base_affichage["text"] = f"Palette rentrée : {liste_de_couleurs_utilisables_de_base}"
            ...
            try:
                ...
                if len(base) == 1 and set(base) != {(255, 255, 255)}:
                    ...
                    print(liste_de_couleurs_utilisables_ajoutées)
                    return
                elif set(base) == {(0, 0, 0), (255, 255, 255)}:
                    liste_de_couleurs_utilisables_de_base = list(base)
                    liste_de_couleurs_utilisables = list(base)
                    liste_de_couleurs_utilisables_ajoutées = []
                    list_color_base_affichage["text"] = f"Palette rentrée : {liste_de_couleurs_utilisables_de_base}"
...
# Afficher la palette
def afficher_la_palette():
    global liste_de_couleurs_utilisables_de_base, Position_affichage, liste_de_couleurs_utilisables, liste_de_couleurs_utilisables_ajoutées, list_color_base_affichage, list_color_affichage, list_color_mel_affichage
    print(" ")
    print("#-------------------------------Affichage de la palette-------------------------------#")
    print(" ")
    print("Palette rentrée par l'utilisateur")
    print(liste_de_couleurs_utilisables_de_base)
    print(" ")
    print("Palette de dessin comprennant les mélanges par sythèse soustractive calculées par le logitiel")
    print(liste_de_couleurs_utilisables)
    print(" ")
    print("Mélanges par sythèse soustractive calculées par le logitiel")
    print(liste_de_couleurs_utilisables_ajoutées)
    print(" ")
    print("#-------------------------------Fin de l'affichage de la palette-------------------------------#")
    print(" ")
    list_color_base_affichage["text"] = f"Palette rentrée : {liste_de_couleurs_utilisables_de_base}"
    list_color_affichage["text"] = f"Palette de dessin : {liste_de_couleurs_utilisables}"
    list_color_mel_affichage["text"] = f"Mélanges calculés : {liste_de_couleurs_utilisables_ajoutées}"
    try:
        if Position_affichage["text"].split("A")[1] != "ffichage de la palette-------------------------------#":
            Position_affichage["text"] += ", #-------------------------------Affichage de la palette-------------------------------#"
    except Exception:
        Position_affichage["text"] += ", #-------------------------------Affichage de la palette-------------------------------#"
...
def premier_point(event):
    global clic_x, clic_y, point_de_départ_x, point_de_départ_y, Position_affichage, color, composants_actifs, tracé, liste_des_tracés, activité, identifiant_ligne_droite_temporaire, identifiant_rectangle_temporaire
    activité = 1
    try:
        ...
        clic_x, clic_y = clip_to_zone_de_dessin(event.x, event.y)
    Position_affichage["text"] = f"Pos x : {clic_x}, Pos y : {clic_y}"
...
def en_train_de_dessiner(event):
    global clic_x, clic_y, color, tracé, liste_des_tracés, Position_affichage, taille_du_crayon, point_de_départ_x, point_de_départ_y, identifiant_ligne_droite_temporaire, identifiant_rectangle_temporaire, delta_x, delta_y, type_de_performance
    delta_x, delta_y = clip_to_zone_de_dessin(event.x, event.y)
    Position_affichage["text"] = f"Pos x : {delta_x}, Pos y : {delta_y}"
    ...
    # Ligne droite
        ...
        zone_de_dessin.delete(identifiant_ligne_droite_temporaire)
        identifiant_ligne_droite_temporaire = zone_de_dessin.create_line(point_de_départ_x, point_de_départ_y, delta_x, delta_y, fill = color, width = 2 * (taille_du_crayon + 1) - 1, dash = (1,1))
        
    # Rectangle
    elif composants_actifs[3] == True:
        zone_de_dessin.delete(identifiant_rectangle_temporaire)
        identifiant_rectangle_temporaire = zone_de_dessin.create_rectangle(point_de_départ_x, point_de_départ_y, delta_x, delta_y, outline = color, width = 2 * (taille_du_crayon + 1) - 1, dash = (1,1))
    
    # Remplissage rectangulaire
    elif composants_actifs[4] == True:
        zone_de_dessin.delete(identifiant_rectangle_temporaire)
...
def stop_dessiner(event):
    global point_de_départ_x, color, couleur_moyenne, point_de_départ_y, fenêtre_de_couleur_globale, Position_affichage, tracé, liste_des_tracés, taille_du_crayon, composants_actifs, identifiant_ligne_droite_temporaire, identifiant_rectangle_temporaire, activité
    clic_x, clic_y = clip_to_zone_de_dessin(event.x, event.y)
    Position_affichage["text"] = f"Pos x : {clic_x}, Pos y : {clic_x}"
    # Rajout si variation de la taille du stylo
    points = []
    if taille_du_crayon > 0 and composants_actifs[0] == True: 
        for (identifiant_tracé, points_tracé, couleur) in tracé[:]:
    ...
    # Ligne droite
    elif composants_actifs[2] == True and (clic_x != point_de_départ_x or clic_y != point_de_départ_y):
        # Cas sans variation de la taille du stylo
        if taille_du_crayon == 0:
            zone_de_dessin.delete(identifiant_ligne_droite_temporaire)
            points = []
        ...
        # Cas avec
                    ...
                    for (identifiant_tracé, points_tracé, couleur) in liste[:]:
                        if (position_1[0] - taille_du_crayon <= points_tracé[0][0] <= position_1[0] + taille_du_crayon and position_1[1] - taille_du_crayon <= points_tracé[0][1] <= position_1[1] + taille_du_crayon) and (position_2[0] - taille_du_crayon <= points_tracé[1][0] <= position_2[0] + taille_du_crayon and position_2[1] - taille_du_crayon <= points_tracé[1][1] <= position_2[1] + taille_du_crayon):
                            ...
                            if liste != []:
                                liste_des_tracés.append(liste)
            for (identifiant_tracé, points_tracé, couleur) in tracé[:]:
                if (points_de_construction[0][0][0] - taille_du_crayon <= points_tracé[0][0] <= points_de_construction[0][0][0] + taille_du_crayon and points_de_construction[0][0][1] - taille_du_crayon <= points_tracé[0][1] <= points_de_construction[0][0][1] + taille_du_crayon) and (points_de_construction[0][0][0] - taille_du_crayon <= points_tracé[1][0] <= points_de_construction[0][0][0] + taille_du_crayon and points_de_construction[0][0][1] - taille_du_crayon <= points_tracé[1][1] <= points_de_construction[0][0][1] + taille_du_crayon):
                    tracé.remove((identifiant_tracé, points_tracé, couleur))
                    zone_de_dessin.delete(identifiant_tracé)
            for liste in liste_des_tracés[:]:
                for (identifiant_tracé, points_tracé, couleur) in liste[:]:
            ...
            for (identifiant_tracé, points_tracé, couleur) in tracé[:]:
                if (points_de_construction[-1][1][0] - taille_du_crayon <= points_tracé[0][0] <= points_de_construction[-1][1][0] + taille_du_crayon and points_de_construction[-1][1][1] - taille_du_crayon <= points_tracé[0][1] <= points_de_construction[-1][1][1] + taille_du_crayon) and (points_de_construction[-1][1][0] - taille_du_crayon <= points_tracé[1][0] <= points_de_construction[-1][1][0] + taille_du_crayon and points_de_construction[-1][1][1] - taille_du_crayon <= points_tracé[1][1] <= points_de_construction[-1][1][1] + taille_du_crayon):
                    tracé.remove((identifiant_tracé, points_tracé, couleur))
                    zone_de_dessin.delete(identifiant_tracé)
            for liste in liste_des_tracés[:]:
                for (identifiant_tracé, points_tracé, couleur) in liste[:]:
            ...
            for delta_coté_x in range(2 * taille_du_crayon + 1):
                for delta_coté_y in range(2 * taille_du_crayon + 1):
                    points.append((clip_to_zone_de_dessin(points_de_construction[-1][0][0] - taille_du_crayon + delta_coté_x, points_de_construction[-1][0][1] - taille_du_crayon + delta_coté_y), clip_to_zone_de_dessin(points_de_construction[-1][1][0] - taille_du_crayon + delta_coté_x + 1, points_de_construction[-1][1][1] - taille_du_crayon + delta_coté_y + 1)))
            for delta_points in points:
                id_ligne = zone_de_dessin.create_line(delta_points[0][0], delta_points[0][1], delta_points[1][0], delta_points[1][1], fill = color, width = 1)
                tracé.append((id_ligne, delta_points, color))
                
    # Rectangle
    elif composants_actifs[3] == True and (clic_x != point_de_départ_x or clic_y != point_de_départ_y):
        # Cas sans variation de la taille du stylo
        if taille_du_crayon == 0:
            zone_de_dessin.delete(identifiant_rectangle_temporaire)
            points = []
        ...
        # Cas avec
            ...
            for delta_coté_x in range(abs(clic_x - point_de_départ_x)):
                if clic_x - point_de_départ_x < 0:
                    points_de_construction.append((clip_to_zone_de_dessin(point_de_départ_x - delta_coté_x, clic_y), clip_to_zone_de_dessin(point_de_départ_x - delta_coté_x - 1, clic_y)))
                else:
                    points_de_construction.append((clip_to_zone_de_dessin(point_de_départ_x + delta_coté_x, clic_y), clip_to_zone_de_dessin(point_de_départ_x + delta_coté_x - 1, clic_y)))
            for delta_coté_y in range(abs(clic_y - point_de_départ_y)):
                if clic_y - point_de_départ_y < 0:
                    points_de_construction.append((clip_to_zone_de_dessin(clic_x, clic_y + delta_coté_y), clip_to_zone_de_dessin(clic_x, clic_y + delta_coté_y - 1)))
                else:
                    points_de_construction.append((clip_to_zone_de_dessin(clic_x, clic_y - delta_coté_y), clip_to_zone_de_dessin(clic_x, clic_y - delta_coté_y - 1)))
            for delta_coté_x in range(abs(clic_x - point_de_départ_x)):
                if clic_x - point_de_départ_x < 0:
                    points_de_construction.append((clip_to_zone_de_dessin(clic_x + delta_coté_x, point_de_départ_y), clip_to_zone_de_dessin(clic_x + delta_coté_x - 1, point_de_départ_y)))
                else:
                    points_de_construction.append((clip_to_zone_de_dessin(clic_x - delta_coté_x, point_de_départ_y), clip_to_zone_de_dessin(clic_x - delta_coté_x - 1, point_de_départ_y)))
            for delta_coté_y in range(abs(clic_y - point_de_départ_y)):
                if clic_y - point_de_départ_y < 0:
                    points_de_construction.append((clip_to_zone_de_dessin(point_de_départ_x, point_de_départ_y - delta_coté_y), clip_to_zone_de_dessin(point_de_départ_x, point_de_départ_y - delta_coté_y - 1)))
                else:
                    points_de_construction.append((clip_to_zone_de_dessin(point_de_départ_x, point_de_départ_y + delta_coté_y), clip_to_zone_de_dessin(point_de_départ_x, point_de_départ_y + delta_coté_y - 1)))
            for position_1, position_2 in points_de_construction:
                for delta_coté_x in range(2 * taille_du_crayon): 
    ...
    # Remplissage rectangulaire
    elif composants_actifs[4] == True and (clic_x != point_de_départ_x or clic_y != point_de_départ_y):
        zone_de_dessin.delete(identifiant_rectangle_temporaire)
        points = []
        for delta_coté_x in range(abs(clic_x - point_de_départ_x)):
            for delta_coté_y in range(abs(clic_y - point_de_départ_y)):
    ...
    if composants_actifs[1] != True and len(tracé) != 0:
        if fenêtre_de_couleur_globale["bg"] != "white":
            couleur_actuelle = tuple(int(color.lstrip('#')[i : i + 2], 16) for i in (0, 2, 4))
            couleur_moyenne_fenêtre = tuple(int(fenêtre_de_couleur_globale["bg"].lstrip('#')[i : i + 2], 16) for i in (0, 2, 4))
            pondération_liste_des_tracés = 0
            for élément in liste_des_tracés:
                pondération_liste_des_tracés += len(élément)
            fenêtre_de_couleur_globale["bg"] = '#%02x%02x%02x' % (round((1 - ((1 - couleur_actuelle[0] / 255) * len(tracé) + (1 - couleur_moyenne_fenêtre[0] / 255) * pondération_liste_des_tracés) / (len(tracé) + pondération_liste_des_tracés)) * 255), round((1 - ((1 - couleur_actuelle[1] / 255) * len(tracé) + (1 - couleur_moyenne_fenêtre[1] / 255) * pondération_liste_des_tracés) / (len(tracé) + pondération_liste_des_tracés)) * 255), round((1 - ((1 - couleur_actuelle[2] / 255) * len(tracé) + (1 - couleur_moyenne_fenêtre[2] / 255) * pondération_liste_des_tracés) / (len(tracé) + pondération_liste_des_tracés)) * 255))
        else:
            fenêtre_de_couleur_globale["bg"] = color
        couleur_moyenne["text"] = f'Couleur moyenne : {fenêtre_de_couleur_globale["bg"]}'
...
def image_en_couleur():
    global liste_de_couleurs_utilisables_de_base, color, liste_de_couleurs_utilisables_ajoutées, liste_de_couleurs_utilisables, list_color_base_affichage, list_color_affichage, list_color_mel_affichage, fenêtre_de_palette_1, fenêtre_de_palette_2, fenêtre_de_palette_3, fenêtre_de_palette_4, fenêtre_de_palette_5, fenêtre_de_palette_6, fenêtre_de_palette_7
    if not liste_de_couleurs_utilisables_de_base:
        liste_de_couleurs_utilisables = []
        liste_de_couleurs_utilisables_de_base = []
        liste_de_couleurs_utilisables_ajoutées = []
...
def image_en_couleur_suite():
    global liste_des_tracés, taille_de_la_feuille_x, taille_de_la_feuille_y, fenêtre_de_couleur_globale, couleur_moyenne
    ...
    for x in range(img.size[0]):
        ...
        if tracé:
            ...
            tracé = []
    couleur = []
    pondération_couleur = 1
    for élément in liste_des_tracés:
        pondération_liste_des_tracés = len(élément)
        couleur_élément = tuple(int(élément[0][2].lstrip('#')[i : i + 2], 16) for i in (0, 2, 4))
        try:
            couleur = (round((1 - ((1 - couleur[0] / 255) * pondération_couleur + (1 - couleur_élément[0] / 255) * pondération_liste_des_tracés) / (pondération_couleur + pondération_liste_des_tracés)) * 255), round((1 - ((1 - couleur[1] / 255) * pondération_couleur + (1 - couleur_élément[1] / 255) * pondération_liste_des_tracés) / (pondération_couleur + pondération_liste_des_tracés)) * 255), round((1 - ((1 - couleur[2] / 255) * pondération_couleur + (1 - couleur_élément[2] / 255) * pondération_liste_des_tracés) / (pondération_couleur + pondération_liste_des_tracés)) * 255))
        except Exception:
            couleur = couleur_élément
        pondération_couleur += 1
    fenêtre_de_couleur_globale["bg"] = '#%02x%02x%02x' % couleur
    couleur_moyenne["text"] = f'Couleur moyenne : {fenêtre_de_couleur_globale["bg"]}'
...
def générer_le_fichier_Gcode():
    global type_du_fichier_d_exportation, activité, Position_affichage, nom_du_fichier_d_exportation, liste_des_tracés_gcode, nombre_de_fichiers_générés, taille_de_la_feuille_x, taille_de_la_feuille_y, liste_des_tracés
    ...
    activité == 0
    try:
        if Position_affichage["text"].split("E")[1] != "xport réussi":
            Position_affichage["text"] += ", Export réussi"
    except Exception:
        Position_affichage["text"] += ", Export réussi"
    

# Texte
Position_affichage = Label(fenêtre, text = "Pos x : 0, Pos y : 0")
Position_affichage.place(x = taille_de_la_feuille_x + 4, y = taille_de_la_feuille_y - 18)
list_color_base_affichage = Label(fenêtre, text = "None")
list_color_base_affichage.place(x = taille_de_la_feuille_x + 4, y = taille_de_la_feuille_y - 96)
list_color_affichage = Label(fenêtre, text = "None")
list_color_affichage.place(x = taille_de_la_feuille_x + 4, y = taille_de_la_feuille_y - 70)
list_color_mel_affichage = Label(fenêtre, text = "None")
list_color_mel_affichage.place(x = taille_de_la_feuille_x + 4, y = taille_de_la_feuille_y - 44)
hex_couleur = Label(fenêtre, text = f"{color}")
hex_couleur.place(x = taille_de_la_feuille_x + 278, y = 30)
Label(fenêtre, text = "  /\ ").place(x = taille_de_la_feuille_x + 86, y = 394)
couleur_moyenne = Label(fenêtre, text = "Couleur moyenne : #ffffff")
couleur_moyenne.place(x = taille_de_la_feuille_x + 4, y = taille_de_la_feuille_y + 3)
...
tk.Button(fenêtre, text = "Utiliser la couleur suivante de la palette", command = utiliser_couleur_palette_suivante).place(x = taille_de_la_feuille_x + 2, y = 262)
tk.Button(fenêtre, text = "Utiliser la couleur précédente de la palette", command = utiliser_couleur_palette_précédente).place(x = taille_de_la_feuille_x + 2, y = 288)
tk.Button(fenêtre, text = "Ajouter la couleur actuelle à la palette", command = ajouter_couleur_à_la_palette).place(x = taille_de_la_feuille_x + 2, y = 314)
tk.Button(fenêtre, text = "Afficher la palette", command = afficher_la_palette).place(x = taille_de_la_feuille_x + 2, y = 340)
...
fenêtre.bind("<N>", lambda event : noir())
fenêtre.bind("<Right>", lambda event : utiliser_couleur_palette_suivante())
fenêtre.bind("<Left>", lambda event : utiliser_couleur_palette_précédente())
fenêtre.bind("<Down>", lambda event : ajouter_couleur_à_la_palette())
fenêtre.bind("<Pause>", lambda event : afficher_la_palette())
fenêtre.bind("<Up>", lambda event : retirer_couleur_à_la_palette())
